...
function encryptedString(key,t) {
    var e, o, s, h, c, i, f, u, v, l, y;
    ...
    for (e = a.length,
    o = "",
    r = 0; r < e; r += key.chunkSize) {
        for (c = new BigInt,
        s = 0,
        f = r + key.chunkSize > e ? e % key.chunkSize : key.chunkSize,
        u = [],
        i = 0; i < f; i++)
            u[i] = a[r + f - 1 - i];
        for (u[f] = 0,
        v = Math.max(8, key.digitSize - 3 - f),
        i = 0; i < v; i++)
            u[f + 1 + i] = Math.floor(Math.random() * 254) + 1;
        for (u[key.digitSize - 2] = 2,
        u[key.digitSize - 1] = 0,
        h = 0; h < key.digitSize; ++s)
            c.digits[s] = u[h++],
            c.digits[s] += u[h++] << 8;
        l = key.barrett.powMod(c, key.e);
        y = key.radix == 16 ? biToHex(l) : biToString(l, key.radix);
        o += y + " "
    }
    return o.substring(0, o.length - 1)
}

function xl(pwd,n,i,t){
    var key = new RSAKeyPair(n,i,t);
    return encryptedString(key,pwd)
}
// console.log(encryptedString("13750430781a"))